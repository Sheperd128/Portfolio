import { assets, serviceData } from '@/assets/assets'
import Image from 'next/image'
import React from 'react'

const Services = () => {
  return (
    <div id='services' className='w-full px-[12%] py-10 scroll-mt-20'>
        <h4 className='text-center mb-2 text-lg font-ovo'>My Projects</h4>
        <h2 className='text-center text-5xl font-ovo'> My latest Work</h2>

        {/* FIXED: Changed 'max-auto' to 'mx-auto' */}
        <p className='text-center max-w-2xl mx-auto mt-5 mb-12 font-ovo'>
            Here are some of my recent projects that showcase my skills and expertise in full stack development and data science.
        </p>

        <div className='grid grid-cols-4  gap-6 my-10'>
            {serviceData.map(({icon, title, description, link}, index) => (
                <div className='border border-gray-400 rounded-lg px-8 py-12 cursor-pointer
                 hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black'
                  key={index}>
                    <div className='flex items-center justify-center mb-4'>
                        <Image src={icon} alt=''className='w-12 h-12' />
                    </div>
                    <h3 className='text-lg my-4 text-grey-700'>{title}</h3>
                    <p className='text-sm text-grey-600 leading-5'>{description}</p>

                    <a href={link} className='flex items-center gap-2 text-sm '>
                        Read More <Image src={assets.right_arrow} alt='' className='w-4'/>
                    </a>
                    </div>
            ))}
        </div>
    </div>
  )
}

export default Services