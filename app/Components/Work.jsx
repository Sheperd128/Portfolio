  ...
  return (
    <div id='work' className='w-full px-[12%] py-20'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
            {workData.map((project, index) => (
                <motion.div 
                    ...
                    key={index}
                    className='group relative rounded-2xl overflow-hidden cursor-pointer h-[400px] border border-white/5 bg-slate-900'
                >
                    ...
                    <div className='absolute bottom-0 left-0 w-full p-6 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500'>
                        ...
                        {/* Conditional Rendering: Link vs Coming Soon */}
                        {project.comingSoon ? (
        ...
        <div className='text-center mt-20'>
             <a href="https://github.com/Sheperd128" target="_blank" className="px-8 py-3 border border-white/20 rounded-full hover:bg-emerald-500/10 hover:border-emerald-500 hover:text-emerald-400 transition duration-300">
                See More on GitHub
             </a>
        </div>
    </div>
